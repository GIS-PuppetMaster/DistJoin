...
from model import made as made
import numpy as np
import torch
...
def train_background(exp_mark):
    if not os.path.exists('./log'):
        os.makedirs('./log')
    command = "nohup " + sys.executable + f" train.py --exp_mark {exp_mark} >./log/{exp_mark}_train.txt" " &"
    print(command)
    os.system(command)


def eval_background(exp_mark):
    if not os.path.exists('./log'):
        os.makedirs('./log')
    command = "nohup " + sys.executable + f" eval-IMDB.py --exp_mark {exp_mark} >./log/{exp_mark}_eval.txt" " &"
    print(command)
    os.system(command)
