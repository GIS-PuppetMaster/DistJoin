...
if __name__ == '__main__':
    ...
    for tag in tags:
        ...
        for workload in train_workloads:
            ...
            queries_job_format = util.JobToQuery(f'/mnt/d/PycharmProjects/learnedcardinalities/data/{workload}.csv')
            ...
            for how in hows:
                if os.path.exists(f'{workload}_{how}{tag}.pkl'):
                    with open(f'{workload}_{how}{tag}.pkl', 'rb') as f:
                        ...
                        print(f'loaded true_card from {workload}_{how}{tag}.pkl')
                else:
                    true_card_list = []
                ...
                df = pd.read_csv(f'/mnt/d/PycharmProjects/learnedcardinalities/data/{workload}.csv', delimiter='#', header=None)
            ...
            df_all.to_csv(f'./MSCN/train.csv', index=False, header=False, sep='#', quoting=csv.QUOTE_NONE)
            with open(f'./MSCN/train-bitmaps.pkl', 'wb') as f:
                pkl.dump(join_bitmaps, f)
            print(f'cost: {(time.time()-start_time)/60} min')