...
def RunN(tables_dict,
    ...
    true_card_file = f'./queries/{workload_tag}_{how}{tag}.pkl'
    if os.path.exists(true_card_file):
        with open(true_card_file, 'rb') as f:
            true_card_list = pkl.load(f)
        print(f'loaded true card file: {true_card_file}')
    ...
    prec_summary = {
        ...
        'GPU memory peak (GB)': max_memory_usage,
    }
    os.makedirs('./result/',exist_ok=True)
    print(prec_summary)
    summary.update(prec_summary)

    q_df['max_gpu_memory_used'] = max_memory_usage
...
def Main(exp_mark):
    ...
    util.gpu_id = 0
    multi = re.match('\{\}-seed[0-9]+-best\.pt', config['glob']) == None
